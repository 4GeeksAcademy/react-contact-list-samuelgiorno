...
export const StoreProvider = ({ children }) => {
  ...
  const fetchContacts = async () => {
    try {
      const check = await fetch('https://playground.4geeks.com/contact/agendas/samuelgiorno/contacts');
      if (check.status === 404) {
    ...
    try {
      const response = await fetch('https://playground.4geeks.com/contact/agendas/samuelgiorno/contacts', {
  ...
  const updateContact = async (id, updatedContact) => {
    try {
      const response = await fetch(`https://playground.4geeks.com/contact/agendas/samuelgiorno/contacts/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const deleteContact = async (id) => {
    try {
      await fetch(`https://playground.4geeks.com/contact/agendas/samuelgiorno/contacts/${id}`, {
        method: 'DELETE',
        headers: {
          'Accept': 'application/json',
        },
